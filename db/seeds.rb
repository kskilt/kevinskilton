...
post.assign_attributes(
  ...
  body: <<~MARKDOWN
    ...
    - Fixed, it flashed — then boot looped with `invalid header` errors. For a few rounds of "clean rebuild, wipe everything and redownload on a second laptop, still broken," it looked like the fix somehow hadn't taken. Turned out I'd been re-pasting the old compile-error log by mistake instead of the actual runtime error — a dumb, human reason to lose an hour, and a good reminder to double check what you're actually looking at before trusting a diagnosis.
    ...
    - Once on the right architecture, the display finally lit up with a calibration screen. Touch didn't respond. What followed was two more laptops' worth of systematically testing IRQ pins (36, 21, 18, 22, 4 — none worked), a detour into swapping to a capacitive GT911 touch chip (wrong guess, more SPI conflicts, a corrupted-flash scare that wiped the calibration screen entirely), and finally going back to basics: matching the *exact* SPI configuration from the working 2.8" resistive board (`SPI3_HOST`, IRQ on pin 36, 2MHz clock, queue depth 3) instead of guessing at new ones. Along the way, the symptom itself became the signal: a stable calibration screen meant the display path was still intact, a flashing white screen meant the latest change had introduced a new conflict — which was usually enough to tell a wrong guess from an actual regression.
    - With that config, plus `TOUCH_SWAP_XY=true`, touch worked.

    ## Cleaning up on purpose

    The maintainer's comment about AI-generated cruft stuck with me. Once the board actually worked, I went back through the branch and stripped out the artificially-generated docs, the test file that didn't test anything, and anything that wasn't the actual fix — matching how every other board in the project is contributed (just a config file and a README entry, no dedicated test suite, which I confirmed by checking the project's own conventions first).

    I prepared a clean PR with just that. I never actually got around to reopening it — the working fix lives in [my fork](https://github.com/kskilt/CYD-Klipper-3.5Resistive), not upstream. It's been running on my printer daily since.
